...
class EditAdressViewController: BaseViewController {
    ...
    fileprivate func buildNavigationItem() {
        ...
        let rightItemButton = UIBarButtonItem.barButton("保存", titleColor: UIColor.lightGray, target: self, action: "saveButtonClick")
    ...
    fileprivate func buildInputView() -> UIView {
        ...
        cancleButton.tag = 10
        cancleButton.addTarget(self, action: "selectedCityTextFieldDidChange:", for: .touchUpInside)
        cancleButton.setTitle("取消", for: UIControlState())
        cancleButton.setTitleColor(UIColor.colorWithCustom(82, g: 188, b: 248), for: UIControlState())
        ...
        determineButton.addTarget(self, action: "selectedCityTextFieldDidChange:", for: .touchUpInside)
        determineButton.setTitleColor(UIColor.colorWithCustom(82, g: 188, b: 248), for: UIControlState())
    ...
    fileprivate func buildGenderButton(_ button: LeftImageRightTextButton, frame: CGRect, title: String, tag: Int) {
        button.tag = tag
        button.setImage(UIImage(named: "v2_noselected"), for: UIControlState())
        button.setImage(UIImage(named: "v2_selected"), for: UIControlState.selected)
        button.addTarget(self, action: "genderButtonClick:", for: .touchUpInside)
        button.setTitle(title, for: UIControlState())
        button.frame = frame
        button.setTitleColor(LFBTextBlackColor, for: UIControlState())
