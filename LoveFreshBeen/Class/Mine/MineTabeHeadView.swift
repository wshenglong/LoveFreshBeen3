...
class MineTabeHeadView: UIView {
    ...
    func click(_ tap: UIGestureRecognizer) {
    ...
    fileprivate func buildUI() {
        ...
        couponNumber?.setBackgroundImage(UIImage(named: "redCycle"), for: UIControlState())
        couponNumber?.setTitleColor(UIColor.red, for: UIControlState())
    ...
    func setCouponNumer(_ number: Int) {
        if number > 0 && number <= 9 {
            ...
            couponNumber?.setTitle("\(number)", for: UIControlState())
...
class MineUpImageDownText: UpImageDownTextButton {
    ...
    init(frame: CGRect, title: String, imageName: String) {
        ...
        setTitleColor(UIColor.colorWithCustom(80, g: 80, b: 80), for: UIControlState())
