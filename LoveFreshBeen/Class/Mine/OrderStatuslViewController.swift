...
class OrderStatuslViewController: BaseViewController {
        ...
        didSet {
            ...
            if order?.detail_buttons?.count > 0 {
                ...
                for i in 0..<order!.detail_buttons!.count {
                    ...
                    btn.setTitle(order!.detail_buttons![i].text, for: UIControlState())
                    btn.backgroundColor = LFBNavigationYellowColor
                    btn.setTitleColor(UIColor.black, for: UIControlState())
                    btn.titleLabel?.font = UIFont.systemFont(ofSize: 13)
                    btn.layer.cornerRadius = 5;
                    btn.tag = order!.detail_buttons![i].type
                    btn.addTarget(self, action: "detailButtonClick:", for: UIControlEvents.touchUpInside)
                    view.addSubview(btn)
                }
            }
        }
    ...
    func detailButtonClick(_ sender: UIButton) {
