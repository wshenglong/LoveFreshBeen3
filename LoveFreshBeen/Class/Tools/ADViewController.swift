...
class ADViewController: UIViewController {
    ...
    var imageName: String? {
        didSet {
            ...
            backImageView.sd_setImage(with: URL(string: imageName!), placeholderImage: UIImage(named: placeholderImageName!), options:SDWebImageOptions(rawValue: 0)) { (image: UIImage?, error: Error?, _, _) in
                if error != nil {
                    //加载广告失败
                    print("加载广告失败")
